...
import { collection, doc, serverTimestamp, setDoc, query, orderBy, getDocs } from "firebase/firestore"
import type { Review, Instructor } from "@/types"
...
const processCourseData = (data: z.infer<typeof courseFormSchema>, instructorData: Instructor, imageUrl: string, videoUrl: string, sortOrder: number) => {
...
export function CreateCourseForm() {
  ...
  const [newInstructorAvatarFile, setNewInstructorAvatarFile] = useState<File | null>(null);
  const [instructorAvatarPreviewUrl, setInstructorAvatarPreviewUrl] = useState<string | null>(null);
  ...
  const instructorsQuery = useMemoFirebase(() => {
    ...
    return query(collection(firestore, 'instructors'), orderBy('name', 'asc'));
  }, [firestore]);

  const { data: instructors, isLoading: isLoadingInstructors } = useCollection<Instructor>(instructorsQuery);
  ...
  const handleSave = (status: "Published" | "Draft") => async (values: z.infer<typeof courseFormSchema>) => {
    ...
    try {
        if (!user || !firestore || !storage || !instructors) {
        ...
        const webinarsCollectionRef = collection(firestore, 'webinars');
        const webinarsSnapshot = await getDocs(webinarsCollectionRef);
        const newSortOrder = webinarsSnapshot.size;

        let instructorData: Instructor;
        if (values.newInstructorName) {
            const instructorSlug = generateSlug(values.newInstructorName);
            let instructorAvatarUrl = 'https://placehold.co/400x400/e2e8f0/e2e8f0?text=No+Image';
            
            if (newInstructorAvatarFile) {
                setIsUploadingInstructorAvatar(true);
                const avatarPath = `hero-images/instructor-photos/${instructorSlug}/${newInstructorAvatarFile.name}`;
                const avatarRef = ref(storage, avatarPath);
                await uploadBytes(avatarRef, newInstructorAvatarFile);
                instructorAvatarUrl = await getDownloadURL(avatarRef);
                setIsUploadingInstructorAvatar(false);
            }
            ...
            instructorData = {
                ...
                bio: 'Newly added instructor.',
                avatarUrl: instructorAvatarUrl,
                sortOrder: (instructors?.length || 0)
            };
            const instructorRef = doc(firestore, 'instructors', instructorSlug);
            await setDoc(instructorRef, instructorData, { merge: true });
            
        } else if (values.instructorId) {
            const selectedInstructor = instructors.find(p => p.id === values.instructorId);
        ...
        const dataToSubmit = processCourseData({ ...values, status }, instructorData, imageUrl, videoUrl, newSortOrder);
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>, type: 'image' | 'video' | 'instructorAvatar') => {
      if (e.target.files && e.target.files[0]) {
        ...
        } else if (type === 'instructorAvatar') {
             if (!file.type.startsWith('image/')) {
                toast({ title: 'Invalid File', description: 'Please select an image file.', variant: 'destructive'});
                return;
            }
            setNewInstructorAvatarFile(file);
            setInstructorAvatarPreviewUrl(preview);
        }
      }
  };
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                     ...
                     <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            render={({ field }) => (
                                ...
                                <Select onValueChange={field.onChange} value={field.value || ""} disabled={totalLoading}>
                                    <FormControl>
                                    <SelectTrigger>
                                        <SelectValue placeholder={isLoadingInstructors ? "Loading..." : "Select an existing instructor"} />
                                    </SelectTrigger>
                                    </FormControl>
                                    <SelectContent>
                                    {instructors?.map(p => (
                        ...
                        {selectedInstructor && (
                            <div className="flex items-center gap-4 mt-2 p-3 border rounded-md bg-muted/50">
                                <Avatar className="h-12 w-12">
                                    <AvatarImage src={selectedInstructor.avatarUrl} alt={selectedInstructor.name} className="object-cover" />
                                    <AvatarFallback>{getInitials(selectedInstructor.name)}</AvatarFallback>
                                </Avatar>
                                <div>
                                    <p className="font-semibold">{selectedInstructor.name}</p>
                                    <p className="text-sm text-muted-foreground">{selectedInstructor.title}</p>
                                </div>
                            </div>
                        )}
                    </div>
                        ...
                        <FormItem>
                            <FormLabel>New Instructor Avatar</FormLabel>
                             <div className="flex items-center gap-4">
                                ...
                                <Input ref={instructorAvatarInputRef} type="file" className="hidden" accept="image/*" onChange={(e) => handleFileSelect(e, 'instructorAvatar')} disabled={totalLoading || !!watchedInstructorId} />
                            </div>
                        </FormItem>
                    </div>
