  ...
  const handleSave = (status: "Published" | "Draft") => async (values: z.infer<typeof courseFormSchema>) => {
    ...
    try {
        ...
        if (values.discountType && values.discountType !== 'none') {
            dataToUpdate.discountType = values.discountType;
            dataToUpdate.discountValue = values.discountValue;
        } else {
            dataToUpdate.discountType = 'none';
            dataToUpdate.discountValue = null; // Use null for deletion
        }
